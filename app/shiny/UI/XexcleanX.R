# DATA EXPLORE and CLEANyou can run the application directly from Github by runing this link at R Console
##############################################################################
...
sidebar <- dashboardSidebar(
    useShinyjs(),
    sidebarMenu(id = "tabs") # sidebar menu
)
...
body <- dashboardBody(
    ...
    # Apply custom shadow to dropdown menu using CSS instead of setShadow
    tags$head(
        tags$style(HTML("
            .dropdown-menu {
    ...
    # Adding this line to fix misaligned header row, keeping the fixed column widths
    tags$head(tags$style(".datatables .display {margin-left: 2;{margin-right: 2;}")),

    # Scroll bar in box
    tags$head(
        tags$style(HTML("
            .box { height: 90vh; overflow-y: auto; font-size:12px; font-style:normal; font-family: 'Lucida Sans Unicode'; }
        "))
    ),
    tags$head(
        tags$style(
...
server <- function(input, output, session) {
    ...
    if (file.exists("../fetched_data/main.rds")) {
        vmy$mydata <- readRDS("../fetched_data/main.rds")
    } else {
        vmy$mydata <- NULL
    }
    ##############################################################################
    ...
    observeEvent(input$mOverviewBtn, {
        showModal(
            mymodal(
                ...
                size = "l",
                column(
                    style = "border: 4px double red;height: 550px;overflow-y: none;",
                    width = 12,
                    align = "center",
        ...
        column(
            ...
            tags$div(
                tags$p(
                    ...
                    HTML(paste(
                        "<h5>", "In this Shiny App demonstrats how to", "<br><h5>",
    ...
    output$mDataExplorationUI <- renderUI({
        ...
        column(
            ...
            align = "center",
            column(
                style = "border: 4px double red;height: 550px;overflow-y: none;background-color:#add8e6;color:black;",
                width = 4,
                align = "center",
            ...
            column(
                style = "border: 4px double red;height: 550px;overflow-y: none;background:#add8e6;color:black;",
                width = 8,
                align = "center",
                # br(),br(),
    ...
    observeEvent(input$mgetfileclick, {
        if (file.exists("../fetched_data/main.rds")) {
            mydata <- as.data.frame(readRDS("../fetched_data/main.rds"))
        } else {
            shinyalert("Oops!", "The file ../fetched_data/main.rds does not exist.", type = "error")
            return()
        }
        ...
        if (ext != "csv" & ext != "rds" & ext != "xlsx" & ext != "xlsx") {
            ...
            tryCatch(
                { # avoid rscript showing error initially before execution,Skipping error in for-loop ::I got from this site; Thank you:https://stackoverflow.com/questions/14748557/skipping-error-in-for-loop
    ...
    output$dt <- DT::renderDataTable({
        DT::datatable(vmy$df_types,
            ...
            options = list(
                scrollY = "275px", dom = "t", ordering = F, pageLength = -1, class = "compact",
                initComplete = JS(
            ...
            if (length(vmy$df_types[input$dt_cell_clicked$row, 1]) > 0) {
                ...
                if (sapply(vmy$mydata[n], is.Date) == TRUE) {
                    ...
                    modalDialog(
                        title = "Warning",
                        selectInput(
                            inputId = "mDateFormat", label = "Select Date format of your Dataset:",
    ...
    mymodal <- function(..., title = NULL, footer = modalButton("Dismiss"),
        ...
        div(
            id = "shiny-modal", class = cls, tabindex = "-1", `data-backdrop` = if (!easyClose) {
                "static"
            }, `data-keyboard` = if (!easyClose) {
                "false"
            }, div(
                class = paste("modal-dialog", idcss), class = switch(size,
                    s = "modal-sm",
                    m = NULL,
                    l = "modal-lg"
                ),
    ...
    observeEvent(input$mExploreStatBtn, {
        ...
        showModal(
            mymodal(
                ...
                size = "l",
                column(
                    style = "border: 4px double red;height: 550px;overflow-y: auto;",
                    width = 12,
                    align = "center",
...
overflow-y:scroll; height: 450px; background: #ffffcd;}")),
        ...
        output$mDescriptiveStat <- renderUI({
            HTML(kbl(temp6,
                escape = FALSE, align = c(rep("c", times = n)), # align center all columns including header in table align=c(rep('c',times=n)) https://stackoverflow.com/questions/41365502/aligning-columns-with-knitr-kable-function
    ...
    output$mPlotlyOutlierPlot <- renderPlotly({
        fig <- plot_ly(
            x = vmy$mydata[, input$mradioPlotVarable], type = "box", quartilemethod = "linear",
            ...
            showlegend = FALSE, plot_bgcolor = "#e5ecf6",
...
overflow-y:none; height: 75px;width:600px; background: #ffffcd;}")),
                ...
                sliderIn <- sliderTextInput(
                    inputId = "msliderRangeInput", label = NULL,
    ...
    output$mcorrelationpairpanel <- renderPlot({
        ...
        psych::pairs.panels(temp[, mcolnames],
            ...
            ci = FALSE,
            stars = FALSE
        ) # If TRUE, adds significance level with stars
    })
    ...
    output$mmultiplot <- renderPlot({
        ...
        if (is.numeric(df2[, 1]) == FALSE) {
            ...
            hist(df2[, 1],
                ...
                xlab = paste("Bin of ", input$mpickerMultiPlot)
            ) + geom_point() + geom_smooth() + coord_cartesian() %>%
                abline(
                    v = mean(df2[, 1], na.rm = T),
                    col = "red",
                    lwd = 2
                ) %>%
                abline(
                    v = median(df2[, 1], na.rm = T),
                    col = "black",
                    lwd = 2
                )
    ...
    fnSimpleLineartyTest <- function() {
        ...
        mtext <- paste(
            "Corr Coeff:", mcorcoff, ";p-value of t-stat:", mtstatp, ". ", "\n",
...
overflow-x: auto;overflow-y: auto; height: 350px; background: #ffffd4;}")),
    ...
    observeEvent(input$btnWidetoLong, {
        ...
        if (length(vmy$df_types[input$dt_cell_clicked$row, 1]) == 0) {
            shinyalert(title = "Instruction", text = paste(
                "First you need to select the variable which should be the key for Wide to Long",
