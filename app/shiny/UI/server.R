...
#            fetched_data(data$combined) # Update the reactive value with fetched data

            # Save fetched data to main.rds
 #           saveRDS(data$combined, "../fetched_data/main.rds")
 ...
 #           })
    ...
    observeEvent(input$apply_filters, {
        ...
        output$distPlot <- renderPlotly({
            req(data$filtered)
            plot_ly(data = data$filtered, x = ~date, y = ~estimate, type = "scatter", mode = "lines+markers", 
                    marker = list(color = input$plot_color), # Use selected color
        ...
        output$dynamicPlotOutput <- renderPlotly({
            ...
            if (input$view_by == "Dimension") {
                if (plot_type == "Scatter") {
                    plot_ly(data = plot_data, x = ~dimension, y = ~estimate, type = "scatter", mode = "lines+markers",
                            marker = list(color = input$plot_color),
                            text = ~ paste("Indicator:", indicator_name, "<br>Dimension:", dimension, "<br>Subgroup:", subgroup, "<br>Date:", date, "<br>Estimate:", estimate),
                            hoverinfo = "text")
                } else if (plot_type == "Bar") {
                    plot_ly(data = plot_data, x = ~dimension, y = ~estimate, type = "bar",
                            marker = list(color = input$plot_color),
                            text = ~ paste("Indicator:", indicator_name, "<br>Dimension:", dimension, "<br>Subgroup:", subgroup, "<br>Date:", date, "<br>Estimate:", estimate),
                            hoverinfo = "text")
                }
                ...
                if (plot_type == "Scatter") {
                    plot_ly(data = plot_data, x = ~subgroup, y = ~estimate, type = "scatter", mode = "lines+markers",
                            marker = list(color = input$plot_color),
                            text = ~ paste("Indicator:", indicator_name, "<br>Dimension:", dimension, "<br>Subgroup:", subgroup, "<br>Date:", date, "<br>Estimate:", estimate),
                            hoverinfo = "text")
                } else if (plot_type == "Bar") {
                    plot_ly(data = plot_data, x = ~subgroup, y = ~estimate, type = "bar",
                            marker = list(color = input$plot_color),
                            text = ~ paste("Indicator:", indicator_name, "<br>Dimension:", dimension, "<br>Subgroup:", subgroup, "<br>Date:", date, "<br>Estimate:", estimate),
                            hoverinfo = "text")
                }
    ...
    observe({
        ...
        output$distPlot <- renderPlotly({
            req(data$combined)
            plot_ly(data = data$combined, x = ~date, y = ~estimate, type = "scatter", mode = "lines+markers", 
                    marker = list(color = input$plot_color), # Use selected color
                    text = ~ paste("Indicator:", indicator_name, "<br>Dimension:", dimension, "<br>Subgroup:", subgroup, "<br>Date:", date, "<br>Estimate:", estimate), 
                    hoverinfo = "text")
        })
    ...
    # Ensure the dynamic plot updates when the chart type is changed
