...
format_analytics_data <- function(analytics_data, indicators, org_units, population_data, indicator_map, dimension = "Region") {
  ...
  # Define a mapping list for the settings per indicator
  indicator_settings <- list(
    "AxBNUGGwwXJ" = list(favourable_indicator = 1, indicator_scale = 100, ordered_dimension = 0, subgroup_order = 0, reference_subgroup = 0),
    "qbwqrHsRlr4" = list(favourable_indicator = 0, indicator_scale = 100, ordered_dimension = 1, subgroup_order = 1, reference_subgroup = 1)
  )
  
  
  rows$iso3 <- "ETH"
  
  # Default settings for indicators not in the list
  default_settings <- list(favourable_indicator = 1, indicator_scale = 100, ordered_dimension = 0, subgroup_order = 0, reference_subgroup = 0)
  
  # Apply these settings to the rows data frame
  rows$favourable_indicator <- sapply(rows$Indicator_ID, function(id) {
    if (!is.null(indicator_settings[[id]])) {
      indicator_settings[[id]]$favourable_indicator
    } else {
      default_settings$favourable_indicator
    }
  })
  
  rows$indicator_scale <- sapply(rows$Indicator_ID, function(id) {
    if (!is.null(indicator_settings[[id]])) {
      indicator_settings[[id]]$indicator_scale
    } else {
      default_settings$indicator_scale
    }
  })
  
  rows$ordered_dimension <- sapply(rows$Indicator_ID, function(id) {
    if (!is.null(indicator_settings[[id]])) {
      indicator_settings[[id]]$ordered_dimension
    } else {
      default_settings$ordered_dimension
    }
  })
  
  rows$subgroup_order <- sapply(rows$Indicator_ID, function(id) {
    if (!is.null(indicator_settings[[id]])) {
      indicator_settings[[id]]$subgroup_order
    } else {
      default_settings$subgroup_order
    }
  })
  
  rows$reference_subgroup <- sapply(rows$Indicator_ID, function(id) {
    if (!is.null(indicator_settings[[id]])) {
      indicator_settings[[id]]$reference_subgroup
    } else {
      default_settings$reference_subgroup
    }
  })
...
if (nrow(combined_data) > 0) {
  ...
  write.xlsx(combined_data,
             file = "00_DHIS2_DEV_Data.xlsx",
             sheetName = "Indicators_Data", rowNames = FALSE
  )
  cat("Excel file '00_DHIS2_DEV_Data.xlsx' created successfully.\n")
} else {
